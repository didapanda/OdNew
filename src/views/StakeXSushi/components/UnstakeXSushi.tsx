...
import sbamboo from "../../../assets/img/sbamboo.png";
    ...
    <Card>
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            {<img src={sbamboo} height={64} width={64}/>}
            <Value value={getBalanceNumber(xSushiBalance)}/>
            <Label text="sBAMBOO  Available"/>
          </StyledCardHeader>
          <StyledCardActions>
            <Button
              disabled={!xSushiBalance.toNumber() || pendingTx}
              text={pendingTx ? 'Converting to BAMBOO' : 'Convert to BAMBOO'}
              onClick={async () => {
                setPendingTx(true)
